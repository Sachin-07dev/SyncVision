...
import {
  Video, Users, GraduationCap, PenTool, Clock, TrendingUp, Cpu,
  Plus, ArrowRight, Code, Presentation,
} from 'lucide-react';
    ...
    <div className="p-6 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2">
          <Link href="/meetings"><Button variant="outline" size="sm"><Video className="w-4 h-4 mr-2" /> New Meeting</Button></Link>
          <Link href="/boards"><Button className="bg-gradient-primary shadow-glow" size="sm"><Plus className="w-4 h-4 mr-2" /> New Board</Button></Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <Card className="bg-card/50 border-border/50">
            <CardHeader className="flex flex-row items-center justify-between pb-4">
              ...
              <Link href="/schedule"><Button variant="ghost" size="sm">View All <ArrowRight className="w-4 h-4 ml-1" /></Button></Link>
            </CardHeader>
            <CardContent className="space-y-3">
              {MOCK_SESSIONS.map((session) => (
                <Link key={session.id} href={`/${session.type === 'meeting' ? 'meeting' : session.type === 'interview' ? 'interview' : 'lecture'}/${session.id}`}>
                  <div className="flex items-center gap-4 p-3 rounded-lg hover:bg-muted/30 transition-colors group">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-sm group-hover:text-primary transition-colors truncate">{session.title}</p>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-xs text-muted-foreground">{session.hostName}</span>
                        <span className="text-xs text-muted-foreground">&bull;</span>
                        <span className="text-xs text-muted-foreground">{session.scheduledDuration}min</span>
                      </div>
                    </div>
                    <div className="text-right">
                      <Badge variant="outline" className={`text-[10px] ${sessionTypeColor[session.type]}`}>{session.type}</Badge>
                      <p className="text-xs text-muted-foreground mt-1">{formatRelativeTime(session.startTime)}</p>
                    </div>
                  </div>
                </Link>
              ))}
        ...
        {/* Activity Feed */}
        <Card className="bg-card/50 border-border/50">
          <CardHeader className="pb-4"><CardTitle className="text-lg">Activity</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            {MOCK_ACTIVITY.map((item) => (
              <div key={item.id} className="flex gap-3">
                <Avatar className="w-8 h-8 flex-shrink-0">
                  <AvatarFallback className="bg-primary/20 text-primary text-[10px]">
                    {item.actorName.split(' ').map((n: string) => n[0]).join('')}
                  </AvatarFallback>
                </Avatar>
                <div className="flex-1 min-w-0">
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/boards"><Button variant="ghost" size="sm">View All <ArrowRight className="w-4 h-4 ml-1" /></Button></Link>
        </div>
                ...
                <CardContent className="p-0">
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <span className="text-xs text-muted-foreground">v{board.version}</span>
                    </div>
                    <div className="flex items-center gap-1 mt-2">
                      <Users className="w-3 h-3 text-muted-foreground" />
                      <span className="text-xs text-muted-foreground">{board.collaborators.length} collaborator{board.collaborators.length !== 1 ? 's' : ''}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
